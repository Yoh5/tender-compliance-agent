...
class TestAMatchWithoutAPageIsNotAMatch:
    """Invariant 1, and `coverage.check` enforces it structurally downstream."""

    def test_no_page_means_uncertain(self, library):
        match = resolve(obligation(), [Suggestion(document=ASSURANCE)], library)
        assert match.certain is False
        assert match.citation is None
        assert "no page" in match.reason

    def test_page_zero_is_refused_like_no_page(self, library):
        match = resolve(obligation(), [Suggestion(document=ASSURANCE, page=0)], library)
        assert match.certain is False

    def test_and_the_row_it_produces_is_never_covered(self, library, deadline):
        match = resolve(obligation(), [Suggestion(document=ASSURANCE)], library)
        row = to_row(match, deadline, today=TODAY)
        assert row.status is not Status.COVERED
        assert check([row]) == []
